...
namespace muscle_ns{
  ...
  void Muscle::lengthCallback(const probo_msgs::distance& m_length)  {
    for (int i = 0; i < 3; i++) { //reading length of all 3 muscles
      muscle_length[i] = m_length.length[i];
    ...
    for (int i = 0; i < 3; i++) { //all 3 muscles
      ...
      sphere_list.id= 4 + (3+i)*resolution;
      line_strip.id = 4+i*resolution;
      points.id = 0;
...
//void Muscle::drawMuscle(Eigen::MatrixXf H, int id, float length, float width) {
...
void Muscle::initializeMatrices() {
  ...
  //std::cout<<"Zero: "<<ZERO_ONE<<std::endl;
