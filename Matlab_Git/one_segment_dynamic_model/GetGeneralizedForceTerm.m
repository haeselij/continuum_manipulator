...
function [generalized_force_term] = GetGeneralizedForceTerm(w_H_1h, dw_H_1h__dq1,dw_H_1h__dq2, dw_H_1h__dq3, q_1, q_2, q_3, p_1, p_2, p_3, v_1,v_2,v_3)

r_b = 0.026;
A = 0.0183*0.0183*pi;  
k=  1108.63; %[N/m]!!!!1108.63;
q_0 = 0.3014;
D_damp = 20;
roundn = @(x,n) round(x*10^n)./10^n;
% Define position of muscle in head frame @AMIR WIR KOMMST DU HIER AUF
% r_b/2?!!
h1_r_B1 = [r_b; 0; 0; 1];%[0; r_b/2; 0; 0];
h1_r_B2 = [(sqrt(3)/2)*(r_b); -(r_b)/2; 0; 1]; %[(sqrt(3)/2)*(r_b/2); -(r_b/2)/2; 0; 0];
h1_r_B3 =[-(sqrt(3)/2)*(r_b); -(r_b)/2; 0; 1]; %[-(sqrt(3)/2)*(r_b/2); -(r_b/2)/2; 0; 0];
...
h1_F_passive_B1 = -(k*(q_1 - q_0) + D_damp*v_1)*[0; 0; 1; 0];  
h1_F_passive_B2 = -(k*(q_2 - q_0) + D_damp*v_2)*[0; 0; 1; 0];
h1_F_passive_B3 = -(k*(q_3 - q_0) + D_damp*v_3)*[0; 0; 1; 0];
...
term_q1_chi_1 = (w_F_B1.')*dw_H_1h__dq1*h1_r_B1; % hier stimmt etwas nicht, gibt bei ruhelage immer 0!! auch wenn p ungleich 0 ist.....
term_q1_chi_2 = (w_F_B2.')*dw_H_1h__dq1*h1_r_B2;
term_q1_chi_3 = (w_F_B3.')*dw_H_1h__dq1*h1_r_B3;
%term_z1 = (w_F_B1.')*dw_H_1h__dq1*[0;0;0;1];
...
term_z2 = (w_F_B2.')*dw_H_1h__dq2*[0;0;0;1];
...
term_z3 = (w_F_B3.')*dw_H_1h__dq3*[0;0;0;1];
